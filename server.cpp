...
int main(int argc, char *argv[]){
	...
	struct sockaddr_storage client_addr;  //connection address information
	socklen_t sin_size;
	
	TCPRequestChannel* main_channel = new TCPRequestChannel ("", port);
	while(1) {  // main accept() loop
        sin_size = sizeof(client_addr);
        int towards_client_socket = accept (main_channel->get_socket_fd(), (struct sockaddr *)&client_addr, &sin_size);
        if (towards_client_socket == -1) {
            perror("accept");
            continue;
        }
		TCPRequestChannel* client_channel = new TCPRequestChannel(towards_client_socket);
